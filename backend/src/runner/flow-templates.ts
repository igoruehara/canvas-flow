...
function step(type: string, index: number, patch: Record<string, any> = {}) {
  return {
    id: createId(type),
    type,
    title: '',
...
function edge(source: any, target: any) {
  return { id: createId('edge'), source: source.id, target: target.id };
}

function base(title: string, channel: 'webWidget' | 'whatsapp') {
  return {
    title,
...
export function getCanvasFlowTemplates() {
  return [
