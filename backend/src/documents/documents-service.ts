...
import { MODEL_NAME } from './documents-constants-model';
...
export class DocumentsService implements OnModuleInit {
  ...
  constructor(
    @Inject(MODEL_NAME) private readonly model: Model<CanvasFlowDocumentEntity>,
  ...
  private storageMode(): 'local' | 's3' {
    const configured = String(this.configService.get<string>('CANVAS_FLOW_FILES_STORAGE') || '').trim().toLowerCase();
  ...
  private s3Bucket() {
    return String(this.configService.get<string>('CANVAS_FLOW_FILES_S3_BUCKET') || '').trim();
  }

  private s3Region() {
    return String(
      this.configService.get<string>('CANVAS_FLOW_FILES_S3_REGION') ||
  ...
  private getS3Client() {
    if (!this.s3Client) this.s3Client = new S3Client({ region: this.s3Region() });
    return this.s3Client;
  }

  private localBaseDir() {
    return resolve(
      String(this.configService.get<string>('CANVAS_FLOW_FILES_LOCAL_DIR') || join(process.cwd(), 'tmp', 'canvas-flow-documents')),
    );
  }
  ...
  private safeSegment(value: any, fallback: string) {
  ...
  private fileKey(documentId: string, filename: string, scope: CanvasDocumentScope) {
    return [
      'canvas-flow',
      this.safeSegment(scope.organizationId, 'global'),
      'documents',
      documentId,
      this.safeSegment(basename(filename), 'arquivo.bin'),
    ].join('/');
  }

  private resolveLocalPath(key: string) {
    const baseDir = this.localBaseDir();
    const target = resolve(baseDir, ...String(key || '').split('/').filter(Boolean));
  ...
  private async writeBytes(storage: 'local' | 's3', key: string, buffer: Buffer, mimeType: string) {
    if (storage === 's3') {
      const bucket = this.s3Bucket();
      if (!bucket) throw new BadRequestException('CANVAS_FLOW_FILES_S3_BUCKET precisa estar configurado para usar S3.');
      const response = await this.getS3Client().send(new PutObjectCommand({
        Bucket: bucket,
        Key: key,
        Body: buffer,
    ...
    await fs.mkdir(resolve(localPath, '..'), { recursive: true });
  ...
  private contentTypeFor(format: CanvasArtifactFormat) {
    return {
  ...
  private getReplacementValue(values: Record<string, any>, path: string) {
    if (Object.prototype.hasOwnProperty.call(values || {}, path)) return values[path];
    return String(path || '').split('.').reduce((current: any, key) => current?.[key], values);
  }

  private assertArtifactFormat(value: any): CanvasArtifactFormat {
    const format = String(value || 'txt').toLowerCase();
    if (!['txt', 'md', 'csv', 'json', 'html', 'docx', 'xlsx', 'pdf'].includes(format)) {
      throw new BadRequestException('Formato de artefato invalido. Use txt, md, csv, json, html, docx, xlsx ou pdf.');
    }
    return format as CanvasArtifactFormat;
  }

  private fillTextTemplate(text: string, replacements: Record<string, any>) {
  ...
  private unwrapGeneratedDocumentContent(content: string) {
    const text = String(content || '').trim();
    if (!/^\s*\{[\s\S]*"content"\s*:/.test(text)) return content;
    try {
  ...
  private flattenReplacements(value: any, prefix = '', output: Record<string, any> = {}) {
  ...
  private normalizedText(value: any) {
  ...
  private parseRichDocumentContent(content: string): CanvasRichDocumentBlock[] {
    const blocks: CanvasRichDocumentBlock[] = [];
    const lines = String(content || '').replace(/\r\n?/g, '\n').split('\n');
    let paragraph: string[] = [];
    let codeLines: string[] = [];
    let listItem: { text: string; marker: string } | null = null;
    let inCode = false;
    ...
    const flushListItem = () => {
      if (listItem?.text) blocks.push({ type: 'listItem', ...listItem });
      listItem = null;
    };
  ...
  private renderDocxHeading(text: string, level: number) {
  ...
  private renderDocxParagraph(text: string) {
    const clean = this.inlineText(text);
    if (!clean) return [];
    return [new Paragraph({
      spacing: { after: 160, line: 276 },
      children: [new TextRun({ text: clean, color: '263238', size: 21 })],
    })];
  }

  private renderDocxLabel(label: string, value: string) {
    ...
    return [new Paragraph({
      spacing: { after: 70 },
      children: [
        new TextRun({ text: `${cleanLabel}: `, bold: true, color: '374151', size: 20 }),
        new TextRun({ text: cleanValue, color: '374151', size: 20 }),
      ],
    })];
  }

  private renderDocxListItem(text: string, marker = '') {
    const clean = this.inlineText(text);
    if (!clean) return [];
    return [new Paragraph({
      ...
      spacing: { after: 80 },
      children: [
        new TextRun({ text: `${marker || '\u2022'} `, bold: true, color: '1F6FEB', size: 20 }),
        new TextRun({ text: clean, color: '263238', size: 20 }),
      ],
    })];
  }

  private renderDocxCodeBlock(lines: string[]) {
  ...
  private renderDocxTable(rows: string[][]) {
  ...
  private async renderNewDocx(content: string) {
    const blocks = this.parseRichDocumentContent(content);
    const children = blocks.flatMap((block) => {
      if (block.type === 'heading') return this.renderDocxHeading(block.text, block.level);
      if (block.type === 'label') return this.renderDocxLabel(block.label, block.value);
      if (block.type === 'listItem') return this.renderDocxListItem(block.text, block.marker);
      if (block.type === 'code') return this.renderDocxCodeBlock(block.lines);
      if (block.type === 'table') return this.renderDocxTable(block.rows);
      return this.renderDocxParagraph(block.text);
    });
    return await Packer.toBuffer(new DocxDocument({
      sections: [{
  ...
  private wordParagraphXml(text: any) {
    return String(text ?? '')
      .split(/\r?\n/)
  ...
  private wordTableCellXml(text: any, width: string) {
    return `<w:tc><w:tcPr><w:tcW w:w="${this.escapeXml(width)}" w:type="dxa"/></w:tcPr>${this.wordParagraphXml(text)}</w:tc>`;
  }

  private appendDocxTableColumn(documentXml: string, edit: Extract<CanvasDocxEdit, { type: 'append_table_column' }>) {
    let tableIndex = -1;
    return documentXml.replace(/<w:tbl(?:\s[^>]*)?>[\s\S]*?<\/w:tbl>/g, (tableXml) => {
      tableIndex += 1;
      const requestedIndex = Math.max(0, Number(edit.tableIndex || 0));
      if (edit.allTables !== true && tableIndex !== requestedIndex) return tableXml;
      ...
      updated = updated.replace(
        ...
        `$1$2<w:gridCol w:w="${this.escapeXml(width)}"/>$3`,
      );
      return updated;
    });
  }

  private appendDocxParagraph(documentXml: string, text: string) {
    if (!String(text || '').trim()) return documentXml;
  ...
  private applyDocxEdits(zip: PizZip, edits: CanvasDocxEdit[]) {
    if (!edits.length) return zip;
    const documentFile = zip.file('word/document.xml');
    if (!documentFile) throw new BadRequestException('DOCX sem word/document.xml.');
  ...
  private async renderTemplateDocx(
    ...
    try {
      ...
      const effectiveEdits = edits.length || !String(fallbackContent || '').trim()
        ? edits
        : [{ type: 'append_paragraph', text: fallbackContent } as CanvasDocxEdit];
      return this.applyDocxEdits(zip, effectiveEdits).generate({ type: 'nodebuffer', compression: 'DEFLATE' }) as Buffer;
    } catch (error: any) {
      throw new BadRequestException(`Nao foi possivel preencher o template DOCX: ${error?.message || String(error)}`);
    }
  }
  ...
  private parseDelimitedRows(content: string) {
    ...
    if (!delimiter || delimiter.count <= 0) return null;
    const rows = lines.map((line) => this.parseDelimitedLine(line, delimiter.item)).filter((row) => row.length > 1);
    if (rows.length < 2 || rows.length < Math.max(2, Math.floor(lines.length * 0.6))) return null;
    return rows;
  }

  private rowsFromJsonContent(content: string) {
    try {
  ...
  private structuredRowsForContent(content: string) {
    const blocks = this.parseRichDocumentContent(content);
    const firstTable = blocks.find((block): block is Extract<CanvasRichDocumentBlock, { type: 'table' }> => block.type === 'table');
    return firstTable?.rows || this.rowsFromJsonContent(content) || this.parseDelimitedRows(content);
  }

  private xlsxCellBorder() {
    return {
      top: { style: 'thin' as ExcelJS.BorderStyle, color: { argb: 'CBD5E1' } },
      left: { style: 'thin' as ExcelJS.BorderStyle, color: { argb: 'CBD5E1' } },
      bottom: { style: 'thin' as ExcelJS.BorderStyle, color: { argb: 'CBD5E1' } },
      right: { style: 'thin' as ExcelJS.BorderStyle, color: { argb: 'CBD5E1' } },
    };
  }

  private normalizeXlsxRows(rows: string[][]) {
    const columnCount = Math.max(1, Math.min(50, ...rows.map((row) => row.length)));
    return rows.slice(0, 5000).map((row) => (
      Array.from({ length: columnCount }, (_, index) => this.inlineText(row[index] || ''))
    ));
  }

  private writeXlsxTable(worksheet: ExcelJS.Worksheet, rows: string[][], startRow: number, title = '') {
    const normalizedRows = this.normalizeXlsxRows(rows);
    if (!normalizedRows.length) return startRow;
  ...
  private writeXlsxMergedRow(
    ...
    rowNumber: number,
    text: string,
    style: 'title' | 'heading' | 'body' | 'note' = 'body',
  ) {
    const clean = this.inlineText(text);
    if (!clean) return rowNumber;
    const span = 6;
  ...
  private autosizeXlsxColumns(worksheet: ExcelJS.Worksheet) {
  ...
  private async renderXlsx(content: string) {
    ...
    if (rowNumber === 1) this.writeXlsxMergedRow(worksheet, 1, 'Documento sem conteudo.', 'body');
    this.autosizeXlsxColumns(worksheet);
    return Buffer.from(await workbook.xlsx.writeBuffer());
  }

  private xlsxWorksheet(workbook: ExcelJS.Workbook, edit: CanvasXlsxEdit) {
  ...
  private xlsxDurationValue(value: any) {
    if (typeof value === 'number' && Number.isFinite(value)) return value;
    const match = String(value ?? '').trim().match(/^(\d+):([0-5]\d)(?::([0-5]\d))?$/);
    if (!match) {
      throw new BadRequestException(`Duracao XLSX invalida: "${String(value ?? '')}". Use HH:mm ou HH:mm:ss.`);
    }
    return (Number(match[1]) * 3600 + Number(match[2]) * 60 + Number(match[3] || 0)) / 86400;
  }

  private xlsxCellValue(value: any, valueType?: CanvasXlsxEdit['valueType']) {
    if (valueType === 'duration') return this.xlsxDurationValue(value);
    if (valueType === 'number') {
      const numeric = Number(value);
      if (!Number.isFinite(numeric)) {
        throw new BadRequestException(`Numero XLSX invalido: "${String(value ?? '')}".`);
      }
      return numeric;
    }
    return value ?? '';
  ...
  private setXlsxCellValue(
    cell: ExcelJS.Cell,
    value: any,
    edit: Pick<CanvasXlsxEdit, 'numberFormat' | 'valueType'>,
  ) {
  ...
  private xlsxColumnNumber(worksheet: ExcelJS.Worksheet, column: string | number | undefined, headerRow: number) {
    const numeric = Number(column);
    if (Number.isInteger(numeric) && numeric > 0) return numeric;
    const expectedHeader = String(column || '').trim().toLowerCase();
    if (!expectedHeader) return 1;
    const row = worksheet.getRow(headerRow);
    for (let index = 1; index <= Math.max(worksheet.columnCount, row.cellCount); index += 1) {
      if (String(row.getCell(index).text || '').trim().toLowerCase() === expectedHeader) return index;
    }
    throw new BadRequestException(`A coluna "${String(column || '')}" nao foi encontrada na aba "${worksheet.name}".`);
  }

  private applyXlsxEdits(workbook: ExcelJS.Workbook, edits: CanvasXlsxEdit[]) {
    edits.slice(0, 1000).forEach((edit) => {
      ...
      if (edit.type === 'set_cell') {
        ...
        if (!/^[A-Z]{1,3}[1-9]\d{0,6}$/.test(address)) {
          throw new BadRequestException(`Celula XLSX invalida: "${address}". Use um endereco como C2.`);
        }
  ...
  private async renderTemplateXlsx(
    templateDocumentId: string,
    replacements: Record<string, any>,
    scope: CanvasDocumentScope,
    edits: CanvasXlsxEdit[] = [],
  ) {
    const { buffer } = await this.getFile(templateDocumentId, scope);
    try {
      const workbook = new ExcelJS.Workbook();
      ...
      throw new BadRequestException(`Nao foi possivel preencher o template XLSX: ${error?.message || String(error)}`);
    }
  }
  ...
  private renderCsv(content: string) {
  ...
  private pdfInlineText(value: any) {
    return this.inlineText(value);
  }

  private pdfAvailableWidth(doc: PDFKit.PDFDocument) {
    return doc.page.width - doc.page.margins.left - doc.page.margins.right;
  }

  private ensurePdfSpace(doc: PDFKit.PDFDocument, height: number) {
    const bottom = doc.page.height - doc.page.margins.bottom - 14;
    const nearTop = doc.y <= doc.page.margins.top + 6;
    if (!nearTop && doc.y + height > bottom) doc.addPage();
  }

  private renderPdfHeading(doc: PDFKit.PDFDocument, text: string, level: number) {
    ...
    if (doc.y > doc.page.margins.top + 6) doc.moveDown(before);
  ...
  private renderPdfParagraph(doc: PDFKit.PDFDocument, text: string) {
    const clean = this.pdfInlineText(text);
    if (!clean) return;
    const left = doc.page.margins.left;
    const width = this.pdfAvailableWidth(doc);
  ...
  private renderPdfLabel(doc: PDFKit.PDFDocument, label: string, value: string) {
  ...
  private renderPdfListItem(doc: PDFKit.PDFDocument, text: string, marker = '') {
    const clean = this.pdfInlineText(text);
    if (!clean) return;
    const left = doc.page.margins.left;
  ...
  private renderPdfCodeBlock(doc: PDFKit.PDFDocument, lines: string[]) {
  ...
  private renderPdfTable(doc: PDFKit.PDFDocument, rows: string[][]) {
    const columnCount = Math.max(2, Math.min(8, ...rows.map((row) => row.length)));
    const normalizedRows = rows.slice(0, 200).map((row) => (
      Array.from({ length: columnCount }, (_, index) => this.pdfInlineText(row[index] || ''))
    ));
    const left = doc.page.margins.left;
    const width = this.pdfAvailableWidth(doc);
    ...
    const rowHeight = (row: string[], isHeader: boolean) => {
      doc.font(isHeader ? 'Helvetica-Bold' : 'Helvetica').fontSize(isHeader ? 8.8 : 8.6);
      return Math.max(24, ...row.map((cell) => (
        doc.heightOfString(cell || ' ', { width: columnWidth - paddingX * 2, lineGap: 1 }) + paddingY * 2
      )));
    };

    const drawRow = (row: string[], rowIndex: number, repeatedHeader = false) => {
  ...
  private drawPdfPageDecoration(doc: PDFKit.PDFDocument) {
  ...
  private async renderPdf(content: string) {
    ...
    const blocks = this.parseRichDocumentContent(content);
    blocks.forEach((block) => {
      if (block.type === 'heading') this.renderPdfHeading(doc, block.text, block.level);
      if (block.type === 'paragraph') this.renderPdfParagraph(doc, block.text);
      if (block.type === 'label') this.renderPdfLabel(doc, block.label, block.value);
      if (block.type === 'listItem') this.renderPdfListItem(doc, block.text, block.marker);
      if (block.type === 'code') this.renderPdfCodeBlock(doc, block.lines);
      if (block.type === 'table') this.renderPdfTable(doc, block.rows);
    });
    if (!blocks.length) this.renderPdfParagraph(doc, ' ');
    this.drawPdfPageDecoration(doc);
    doc.end();
    return await completed;
  }

  private async renderArtifact(params: {
    ...
    if (params.format === 'docx') {
      if (params.templateDocumentId) {
        return await this.renderTemplateDocx(
    ...
    if (params.format === 'xlsx') {
      if (params.templateDocumentId) {
        return await this.renderTemplateXlsx(
          params.templateDocumentId,
          params.replacements || {},
          params.scope || {},
          params.xlsxEdits || [],
    ...
    if (params.format === 'pdf') return await this.renderPdf(content);
    if (params.format === 'html') return Buffer.from(this.renderHtml(content), 'utf-8');
    if (params.format === 'csv') return Buffer.from(this.renderCsv(content), 'utf-8');
  ...
  async storeOriginal(params: {
    ...
    const filename = this.safeSegment(basename(params.filename || 'arquivo.bin'), 'arquivo.bin');
    ...
    const row = await new this.model({
      ...
      metadata: {
        ...(params.metadata || {}),
        hashSha256: createHash('sha256').update(params.buffer).digest('hex'),
  ...
  async updateExtraction(documentId: string, extraction: { text?: string; structure?: Record<string, any>; metadata?: Record<string, any> }) {
  ...
  async createArtifact(params: {
    ...
    if (!buffer.length) {
      throw new BadRequestException('Nao foi possivel gerar o artefato: o conteudo resultou em um arquivo vazio.');
    }
    const documentId = randomUUID();
  ...
  async getRecord(documentId: string, scope: CanvasDocumentScope = {}) {
  ...
  async getFile(documentId: string, scope: CanvasDocumentScope = {}) {
    ...
    return { record, buffer: await fs.readFile(this.resolveLocalPath(record.key)) };
  }

  async openLocalReadStream(documentId: string, scope: CanvasDocumentScope = {}) {
    const record = await this.getRecord(documentId, scope);
    if (record.storage !== 'local') return null;
    return { record, stream: createReadStream(this.resolveLocalPath(record.key)) };
  }

  async withSignedDownloadUrl(record: any) {
    if (record.storage !== 's3') {
      const prefix = this.publicApiUrl();
      return { ...record, downloadUrl: `${prefix}${record.downloadPath}` };
    }
    const expiresIn = Math.max(60, Math.min(Number(this.configService.get<string>('CANVAS_FLOW_FILES_DOWNLOAD_TTL_SECONDS') || 900), 86400));
    const downloadUrl = await getSignedUrl(this.getS3Client(), new GetObjectCommand({
      Bucket: record.bucket || this.s3Bucket(),
      Key: record.key,
  ...
  async getDownloadInfo(documentId: string, scope: CanvasDocumentScope = {}) {
    return await this.withSignedDownloadUrl(await this.getRecord(documentId, scope));
  }

  async list(scope: CanvasDocumentScope = {}, limit = 100) {
    const query: Record<string, any> = {};
    if (scope.organizationId) query.organizationId = scope.organizationId;
    if (scope.agentId) query.agentId = scope.agentId;
    if (scope.flowId) query.flowId = scope.flowId;
    if (scope.conversationId) query.conversationId = scope.conversationId;
