...
import { AGENT_MODEL_NAME, MODEL_NAME, VERSION_MODEL_NAME } from './canvas-flow-constants-model';
...
@Injectable()
export class CanvasFlowService {
  constructor(
    @Inject(MODEL_NAME) private model: Model<CanvasFlowEntity>,
    @Inject(AGENT_MODEL_NAME) private agentModel: Model<CanvasFlowAgentEntity>,
    @Inject(VERSION_MODEL_NAME) private versionModel: Model<CanvasFlowVersionEntity>,
  ) {}

  private normalizeAgentName(value: any) {
    return String(value || '').trim() || 'default-agent';
  }

  private createAgentIdSlug(value: any) {
  ...
  private normalizeAgentDisplayName(value: any, fallback?: string) {
    return String(value || fallback || '').trim() || 'Agente';
  }

  private async nextAvailableAgentId(baseAgentId: string, organizationId?: string) {
    ...
    for (let index = 0; index < 1000; index += 1) {
      ...
      if (!agent && !flow) return candidate;
    }
    return `${base}-${Date.now()}`;
  }

  private async unsetOtherMainFlows(flow: any) {
    if (flow?.config?.isMainFlow !== true) return;
    const query: Record<string, any> = {
      _id: { $ne: flow._id },
      'config.isMainFlow': true,
    };
    if (flow.agentId) query.agentId = flow.agentId;
    if (flow.organizationId) query.organizationId = flow.organizationId;
    if (flow.config?.channel) query['config.channel'] = flow.config.channel;
    await this.model.updateMany(query, { $set: { 'config.isMainFlow': false } }).exec();
  }

  private withOrganization(query: Record<string, any>, organizationId?: string) {
    if (organizationId) query.organizationId = organizationId;
    return query;
  }

  private scopedQuery(agentId?: string, organizationId?: string) {
    return this.withOrganization(agentId ? { agentId } : {}, organizationId);
  }

  private agentQuery(agentId?: string, organizationId?: string) {
    const query: Record<string, any> = {};
    if (agentId) {
  ...
  private agentDisplayNameQuery(name?: string, organizationId?: string, excludeAgentId?: string) {
    const query: Record<string, any> = { name: this.normalizeAgentDisplayName(name) };
    if (organizationId) query.organizationId = organizationId;
    if (excludeAgentId) query.agentId = { $ne: this.normalizeAgentName(excludeAgentId) };
    return query;
  }

  private async ensureAgent(agentId?: string, auth?: { organizationId?: string; userId?: string }, displayName?: string) {
    ...
    await this.agentModel.updateOne(
      this.agentQuery(id, auth?.organizationId),
      {
        $set: {
          agentId: id,
        },
  ...
  private normalizeObjectList(value: any): Array<Record<string, any>> {
  ...
  private normalizeBlockedTerms(value: any): string[] {
  ...
  private normalizeAgentRuntimeConfig(value: any): AgentRuntimeConfig {
    ...
    return {
      ...
      ...(allowedProviders.has(provider) ? { llmProvider: provider as AgentRuntimeConfig['llmProvider'] } : {}),
      agentSpec: {
  ...
  private safeWorkspaceFileName(value: any, fallback: string) {
  ...
  private workspaceJson(value: any) {
    return `${JSON.stringify(value ?? {}, null, 2)}\n`;
  }

  private workspaceFile(path: string, content: any, type = 'text/plain'): AgentWorkspaceFile {
    return {
      path,
  ...
  private normalizeWorkspacePath(path: any) {
    return String(path || '')
      .replace(/\\/g, '/')
      .replace(/^\/+/, '')
      .toLowerCase();
  ...
  private workspaceFilesFromPayload(payload: any): AgentWorkspaceFile[] {
  ...
  private readWorkspaceFile(files: AgentWorkspaceFile[], path: string) {
    const target = this.normalizeWorkspacePath(path);
    return files.find((file) => this.normalizeWorkspacePath(file.path) === target)?.content;
  }

  private readWorkspaceFolder(files: AgentWorkspaceFile[], folder: string) {
  ...
  private parseWorkspaceJson(content: any, fallback: any) {
    if (content === undefined || content === null || content === '') return fallback;
  ...
  private normalizeWorkspaceLoadMode(value: any, fallback: 'always' | 'auto' | 'on_demand' | 'manual' = 'auto') {
    const mode = String(value || '').trim();
    if (mode === 'always' || mode === 'auto' || mode === 'on_demand' || mode === 'manual') return mode;
    return fallback;
  }

  private catalogItemId(item: any, fallback: string) {
    return String(item?.id || item?.key || item?.name || item?.label || fallback).trim() || fallback;
  }

  private catalogItemName(item: any, fallback: string) {
    return String(item?.name || item?.label || item?.title || this.catalogItemId(item, fallback)).trim() || fallback;
  }

  private catalogItemDescription(item: any) {
    return String(item?.description || item?.role || item?.instructions || item?.instruction || item?.action || '').trim();
  }

  private buildWorkspaceManifest(config: AgentRuntimeConfig) {
    const spec = config.agentSpec || {};
    const toEntries = (items: any, kind: string, fallbackLoad: 'always' | 'auto' | 'on_demand' | 'manual') => (
      Array.isArray(items) ? items : []
    ).filter((item) => this.isPlainObject(item)).map((item, index) => ({
  ...
  private applyWorkspaceManifestList(list: any, manifestEntries: any, kind: string, fallbackLoad: 'always' | 'auto' | 'on_demand' | 'manual') {
    ...
    if (!entries.length) return current.map((item, index) => ({
      ...item,
      id: this.catalogItemId(item, `${kind}-${index + 1}`),
      load: this.normalizeWorkspaceLoadMode(item.load || item.loadMode, fallbackLoad),
    }));
  ...
  private applyWorkspaceManifestToSpec(spec: any, manifest: any) {
    if (!this.isPlainObject(manifest)) return spec;
    return {
      ...spec,
  ...
  private workspaceConfigFromPayload(payload: any): AgentRuntimeConfig {
    const source = this.isPlainObject(payload?.workspace) ? payload.workspace : payload;
  ...
  private normalizeVersionValue(value: any): number | undefined {
    if (value === undefined || value === null || value === '') return undefined;
    if (String(value).trim().toLowerCase() === 'active') return undefined;
    const version = Number(value);
    return Number.isInteger(version) && version > 0 ? version : undefined;
  }

  private async updateActiveAgentReleaseFlowVersion(flow: any, version: number, auth?: CanvasFlowAuthContext, timestamp = new Date().toISOString()) {
    const agentId = this.normalizeAgentName(flow?.agentId);
    const flowId = flow?._id ? String(flow._id) : '';
    if (!flowId || !agentId || agentId === 'default-agent' || !version) return;
    ...
    await this.agentModel.findOneAndUpdate(
      { ...this.agentQuery(agentId, organizationId), 'releases.release': activeRelease },
      {
        $set: {
  ...
  private flowVersions(flow: any): any[] {
    return Array.isArray(flow?.versions) ? flow.versions : [];
  }

  private flowId(flow: any): string {
    return String(flow?._id || flow?.flowId || flow || '').trim();
  }

  private flowVersionQuery(flowOrId: any, organizationId?: string, version?: number) {
    const query: Record<string, any> = { flowId: this.flowId(flowOrId) };
    if (organizationId) query.organizationId = organizationId;
    if (version) query.version = version;
    return query;
  }

  private flowObjectId(id: any) {
    const value = String(id || '').trim();
    return Types.ObjectId.isValid(value) ? new Types.ObjectId(value) : value;
  }

  private versionRecordFromFlow(flow: any, version: Record<string, any>): Record<string, any> {
    ...
    delete (record as any).createdAt;
    delete (record as any).updatedAt;
    return {
      ...record,
      flowId,
  ...
  private async aggregateVersionRecords(query: Record<string, any>, includeBsonSize = true): Promise<any[]> {
    ...
    if (includeBsonSize) {
      try {
        ...
        // Older Mongo versions may not support $bsonSize; keep the API usable.
      }
    }
    return await this.versionModel.find(query).sort({ version: -1 }).lean().exec();
  }

  private async flowDocumentBsonSize(id: any, organizationId?: string): Promise<number | undefined> {
  ...
  private async versionDocumentsBsonSize(flowOrId: any, organizationId?: string): Promise<number | undefined> {
    const query = this.flowVersionQuery(flowOrId, organizationId);
    if (!query.flowId) return undefined;
    try {
  ...
  private async migrateEmbeddedVersions(
    flow: any,
    organizationId?: string,
    options?: { dryRun?: boolean; keepLegacy?: boolean },
  ): Promise<{ embeddedCount: number; upsertedCount: number }> {
    const embedded = this.flowVersions(flow).filter((version) => Number(version?.version || 0) > 0);
    const flowId = this.flowId(flow);
    if (!flowId || !embedded.length) return { embeddedCount: 0, upsertedCount: 0 };
    ...
    if (!options?.keepLegacy) {
      await this.model.updateOne(
        this.withOrganization({ _id: flow._id }, organizationId || flow?.organizationId),
        { $unset: { versions: '' } },
  ...
  private async loadFlowForVersionAccess(id: string, organizationId?: string): Promise<any> {
    const flow = await this.model
      ...
      .exec();
    if (!flow) {
      throw new HttpException('Canvas flow not found', HttpStatus.NOT_FOUND);
    }
    return flow;
  }

  private async findFlowVersions(flow: any, organizationId?: string, includeBsonSize = true): Promise<any[]> {
    const flowId = this.flowId(flow);
    if (!flowId) return [];
    ...
    return await this.aggregateVersionRecords(
      this.flowVersionQuery(flowId, organizationId || flow?.organizationId),
      includeBsonSize,
    );
  }

  private async findFlowVersion(flow: any, version: number, organizationId?: string): Promise<any | undefined> {
  ...
  private latestVersionFromRecords(flow: any, versions: any[]) {
  ...
  private async latestExistingFlowVersionNumberAsync(flow: any, organizationId?: string) {
    const versions = await this.findFlowVersions(flow, organizationId || flow?.organizationId, false);
    return versions.reduce((max, version) => Math.max(max, Number(version?.version) || 0), 0);
  }

  private async flowWithVersions(flow: any, organizationId?: string): Promise<any> {
  ...
  async migrateEmbeddedFlowVersions(options: CanvasFlowVersionMigrationOptions = {}): Promise<CanvasFlowVersionMigrationSummary> {
    const summary: CanvasFlowVersionMigrationSummary = {
    ...
    const flowQuery = this.model
      ...
      .lean();
    if (limit) flowQuery.limit(limit);
    const cursor = flowQuery.cursor();

    for await (const flow of cursor as any) {
      ...
      if (legacySize !== undefined) summary.legacyFlowBsonSizeBytes = (summary.legacyFlowBsonSizeBytes || 0) + legacySize;
      ...
      try {
        ...
        if (!options.dryRun) {
          ...
          if (postSize !== undefined) summary.postMigrationFlowBsonSizeBytes = (summary.postMigrationFlowBsonSizeBytes || 0) + postSize;
          if (versionSize !== undefined) summary.versionBsonSizeBytes = (summary.versionBsonSizeBytes || 0) + versionSize;
        }
      } catch (error: any) {
  ...
  private agentReleases(agent: any): any[] {
    return Array.isArray(agent?.releases) ? agent.releases : [];
  }

  private async agentReleasesForResponse(agent: any, organizationId?: string, existingFlowIds?: Set<string>): Promise<any[]> {
    const releases = this.agentReleases(agent);
    const snapshots = new Map<string, { flowId: string; version: number }>();
    ...
    const snapshotNames = new Map(
      snapshotRecords
        .filter((record: any) => String(record?.name || '').trim())
        .map((record: any) => [`${String(record.flowId)}:${Number(record.version)}`, String(record.name)]),
    );
    ...
    return releases
      .map((release) => {
        ...
        const nextFlowNames: Record<string, string> = {};
        const nextVersionNames: Record<string, string> = {};
        Object.entries(versions).forEach(([flowId, rawVersion]) => {
          if (existingFlowIds && !existingFlowIds.has(flowId)) return;
          const version = Number(rawVersion);
          nextVersions[flowId] = version;
          if (flowNames[flowId]) nextFlowNames[flowId] = String(flowNames[flowId]);
          const versionName = snapshotNames.get(`${flowId}:${version}`) || storedVersionNames[flowId];
          if (versionName) nextVersionNames[flowId] = String(versionName);
        });
  ...
  resolveFlowVersion(flow: any, requestedVersion?: any): { config: any; version?: number; source: 'draft' | 'version'; activeVersion?: number; latestVersion: number } {
    ...
    if (targetVersion) {
      ...
      if (!snapshot) {
        throw new HttpException(`Versao ${targetVersion} do fluxo nao encontrada.`, HttpStatus.NOT_FOUND);
      }
      return {
  ...
  async resolveFlowVersionAsync(flow: any, requestedVersion?: any): Promise<{ config: any; version?: number; source: 'draft' | 'version'; activeVersion?: number; latestVersion: number }> {
    const rawRequest = String(requestedVersion ?? '').trim().toLowerCase();
    const active = this.normalizeVersionValue(flow?.activeVersion);
    if (rawRequest === 'draft') {
      return {
  ...
  async resolveAgentRelease(agentId?: string, organizationId?: string, requestedRelease?: any): Promise<{ release?: number; versions: Record<string, number>; source: 'active' | 'requested' | 'none'; latestRelease?: number }> {
    ...
    if (!agent) return { versions: {}, source: 'none' };
    ...
    if (!release) {
      ...
      throw new HttpException(`Release ${target} do agente nao encontrado.`, HttpStatus.NOT_FOUND);
    }
    return {
      release: target,
  ...
  private async nextAgentSortOrder(organizationId?: string) {
  ...
  private async nextSortOrder(agentId?: string, organizationId?: string) {
  ...
  async create(createDto: CreateCanvasFlowDto, auth?: { organizationId?: string; userId?: string }): Promise<CanvasFlowEntity> {
    const agentId = this.normalizeAgentName(createDto.agentId);
    if (agentId !== 'default-agent') {
      await this.ensureAgent(agentId, auth);
    }
    const initialVersions = Array.isArray((createDto as any).versions) ? (createDto as any).versions : [];
    const { versions: _ignoredVersions, ...flowDto } = createDto as any;
    const initialLatestVersion = initialVersions.reduce((max: number, version: any) => Math.max(max, Number(version?.version) || 0), 0);
    ...
    if (initialVersions.length) {
      const operations = initialVersions
        .filter((version: any) => Number(version?.version || 0) > 0)
        .map((version: any) => ({
          updateOne: {
  ...
  async findAll(agentId?: string, organizationId?: string, options?: { includeConfig?: boolean }): Promise<any[]> {
  ...
  async listAgents(organizationId?: string): Promise<any[]> {
    ...
    explicitAgents.forEach((agent: any) => {
  ...
  async createAgent(name: string, auth?: { organizationId?: string; userId?: string }) {
    ...
    if (duplicate) {
      throw new HttpException('Ja existe um agente com este nome.', HttpStatus.CONFLICT);
    }
  ...
  async getAgentConfig(agentId: string, organizationId?: string): Promise<AgentRuntimeConfig> {
  ...
  async updateAgentConfig(agentId: string, config: any, auth?: { organizationId?: string; userId?: string }) {
    const target = this.normalizeAgentName(agentId);
    if (!target || target === 'default-agent') {
      throw new HttpException('Selecione um agente real antes de configurar o Agent OS.', HttpStatus.BAD_REQUEST);
    }
    ...
    await this.agentModel.updateOne(
      this.agentQuery(target, auth?.organizationId),
      {
        $set: {
          agentId: target,
  ...
  async exportAgentWorkspace(agentId: string, organizationId?: string): Promise<AgentWorkspacePackage> {
    const target = this.normalizeAgentName(agentId);
    if (!target || target === 'default-agent') {
      throw new HttpException('Selecione um agente real para exportar o workspace.', HttpStatus.BAD_REQUEST);
    }

    const agent = await this.agentModel.findOne(this.agentQuery(target, organizationId)).lean().exec();
    if (!agent) {
      throw new HttpException('Agente nao encontrado para exportar workspace.', HttpStatus.NOT_FOUND);
    }
    ...
    const files: AgentWorkspaceFile[] = [
      ...
      ...rules.map((rule, index) => this.workspaceFile(
        `.canvas-flow/rules/${this.safeWorkspaceFileName(rule.id || rule.name, `rule-${index + 1}`)}.rule.json`,
        rule,
        'application/json',
      )),
      ...skills.map((skill, index) => this.workspaceFile(
        `.canvas-flow/skills/${this.safeWorkspaceFileName(skill.id || skill.name, `skill-${index + 1}`)}.skill.json`,
        skill,
        'application/json',
      )),
      ...subagents.map((subagent, index) => this.workspaceFile(
        `.canvas-flow/subagents/${this.safeWorkspaceFileName(subagent.id || subagent.name, `subagent-${index + 1}`)}.agent.json`,
        subagent,
        'application/json',
      )),
    ];
  ...
  async importAgentWorkspace(agentId: string, payload: any, auth?: { organizationId?: string; userId?: string }) {
    const target = this.normalizeAgentName(agentId);
    if (!target || target === 'default-agent') {
      throw new HttpException('Selecione um agente real antes de importar workspace.', HttpStatus.BAD_REQUEST);
    }
    const config = this.workspaceConfigFromPayload(payload || {});
    return await this.updateAgentConfig(target, config, auth);
  }

  async renameAgent(currentAgentId: string, nextName: string, auth?: { organizationId?: string }) {
    ...
    await this.agentModel.updateOne(
      this.agentQuery(agentId, auth?.organizationId),
      {
  ...
  async removeAgent(agentId: string, confirmationName: string, auth?: { organizationId?: string }) {
    const target = this.normalizeAgentName(agentId);
    ...
    if (confirmation !== displayName && confirmation !== target) {
      throw new HttpException('Digite o nome exato do agente para confirmar a exclusao.', HttpStatus.BAD_REQUEST);
    }
  ...
  async reorderAgents(orderedAgentIds: string[], auth?: { organizationId?: string; userId?: string }): Promise<any[]> {
    const ids = Array.from(new Set(orderedAgentIds.map((id) => this.normalizeAgentName(id)).filter(Boolean)));
    if (!ids.length) return await this.listAgents(auth?.organizationId);
  ...
  async reorder(orderedIds: string[], agentId?: string, organizationId?: string): Promise<any[]> {
    const ids = orderedIds.filter(Boolean);
    if (!ids.length) return await this.findAll(agentId, organizationId);
  ...
  async findMain(agentId?: string, channel?: string): Promise<any> {
    const baseQuery: Record<string, any> = {};
    if (agentId) baseQuery.agentId = agentId;

    const flows = await this.model.find(baseQuery).sort({ updatedAt: -1 }).lean().exec();
    const agentRelease = await this.resolveAgentRelease(agentId).catch(() => ({ versions: {} as Record<string, number>, source: 'none' as const }));
    ...
    const matchesChannel = (item: any) => !channel || item.resolved?.config?.channel === channel;
    const main = resolved.find((item) => matchesChannel(item) && item.resolved?.config?.isMainFlow === true);
    if (main) return main.flow;

    const fallback = resolved.find((item) => matchesChannel(item))?.flow;
    if (!fallback) {
      throw new HttpException('Canvas main flow not found', HttpStatus.NOT_FOUND);
    }
    return fallback;
  }

  async findOne(id: string, organizationId?: string, options?: { includeVersions?: boolean; includeBsonSize?: boolean }): Promise<any> {
    const query = this.withOrganization({ _id: id }, organizationId);
    const request = this.model.findOne(query);
    if (options?.includeVersions) request.select('+versions');
  ...
  async getVersions(id: string, auth?: { organizationId?: string }): Promise<any> {
  ...
  async deployVersion(id: string, body: any = {}, auth?: CanvasFlowAuthContext): Promise<any> {
    const query = this.withOrganization({ _id: id }, auth?.organizationId);
    const flow = await this.loadFlowForVersionAccess(id, auth?.organizationId);

    const updates: Record<string, any> = {};
    if (body?.name !== undefined) updates.name = String(body.name || flow.name || 'Fluxo');
    ...
    await this.versionModel.updateOne(
      this.flowVersionQuery(flow, auth?.organizationId, nextVersion),
      { $setOnInsert: versionRecord },
      { upsert: true },
    ).exec();
  ...
  async activateVersion(id: string, version: any, auth?: CanvasFlowAuthContext): Promise<any> {
    const targetVersion = this.normalizeVersionValue(version);
    if (!targetVersion) {
      throw new HttpException('Versao invalida para ativacao.', HttpStatus.BAD_REQUEST);
    }
    const flow = await this.loadFlowForVersionAccess(id, auth?.organizationId);
    const versionRecord = await this.findFlowVersion(flow, targetVersion, auth?.organizationId);
    if (!versionRecord) {
      throw new HttpException('Versao do fluxo nao encontrada.', HttpStatus.NOT_FOUND);
    }
    const activatedAt = new Date().toISOString();
    await this.versionModel.updateOne(
      this.flowVersionQuery(flow, auth?.organizationId, targetVersion),
      {
        $set: {
          activatedAt,
          activatedBy: auth?.userId || '',
  ...
  async renameVersion(id: string, version: any, body: any = {}, auth?: { organizationId?: string }): Promise<any> {
    const targetVersion = this.normalizeVersionValue(version);
    const name = String(body?.name || '').trim();
    if (!targetVersion) {
      throw new HttpException('Versao invalida para renomear.', HttpStatus.BAD_REQUEST);
    }
    if (!name) {
      throw new HttpException('Informe um nome para a versao.', HttpStatus.BAD_REQUEST);
    }

    const flow = await this.loadFlowForVersionAccess(id, auth?.organizationId);
    const updatedVersion = await this.versionModel.findOneAndUpdate(
      this.flowVersionQuery(flow, auth?.organizationId, targetVersion),
      { $set: { name } },
      { new: true },
    ).lean().exec();
  ...
  async deleteVersion(id: string, version: any, auth?: { organizationId?: string }): Promise<any> {
    const targetVersion = this.normalizeVersionValue(version);
    if (!targetVersion) {
      throw new HttpException('Versao invalida para exclusao.', HttpStatus.BAD_REQUEST);
    }
    ...
    if (this.normalizeVersionValue(flow.activeVersion) === targetVersion) {
      throw new HttpException('Nao e possivel excluir a versao ativa. Ative outra versao antes.', HttpStatus.BAD_REQUEST);
    }
  ...
  async overwriteVersion(id: string, version: any, body: any = {}, auth?: CanvasFlowAuthContext): Promise<any> {
    const targetVersion = this.normalizeVersionValue(version);
    if (!targetVersion) {
      throw new HttpException('Versao invalida para sobrescrita.', HttpStatus.BAD_REQUEST);
    }
    ...
    if (sourceVersion && !sourceSnapshot) {
      throw new HttpException(`Versao ${sourceVersion} do fluxo nao encontrada para origem.`, HttpStatus.NOT_FOUND);
    }
    ...
    if (body?.name !== undefined) set.name = String(body.name || flow.name || `v${targetVersion}`);
    ...
    const updated = await this.model.findOneAndUpdate(
      this.withOrganization({ _id: id }, auth?.organizationId),
      { $unset: { versions: '' } },
      { new: true },
    ).lean().exec();
  ...
  async getAgentReleases(agentId: string, auth?: { organizationId?: string; userId?: string }) {
    const name = this.normalizeAgentName(agentId);
    ...
    const existingFlowIds = new Set(flows.map((flow: any) => String(flow._id)));
  ...
  async deployAgentRelease(agentId: string, body: any = {}, auth?: CanvasFlowAuthContext) {
    const name = this.normalizeAgentName(agentId);
    ...
    if (!flows.length) {
      throw new HttpException('Nenhum fluxo encontrado para este agente.', HttpStatus.BAD_REQUEST);
    }

    const versionMap: Record<string, number> = {};
    const flowNames: Record<string, string> = {};
    const flowOperations: any[] = [];
    const versionOperations: any[] = [];
    const now = new Date().toISOString();
    ...
    if (body?.activate !== false) set.activeRelease = nextRelease;

    const updated = await this.agentModel.findOneAndUpdate(
      this.agentQuery(name, auth?.organizationId),
      {
        $set: set,
        $setOnInsert: {
          name,
  ...
  async activateAgentRelease(agentId: string, release: any, auth?: CanvasFlowAuthContext) {
    const name = this.normalizeAgentName(agentId);
    const targetRelease = this.normalizeVersionValue(release);
    if (!targetRelease) {
      throw new HttpException('Release invalido para ativacao.', HttpStatus.BAD_REQUEST);
    }

    const updated = await this.agentModel.findOneAndUpdate(
      { ...this.agentQuery(name, auth?.organizationId), 'releases.release': targetRelease },
      {
        $set: {
    ...
    if (!updated) {
      throw new HttpException('Release do agente nao encontrado.', HttpStatus.NOT_FOUND);
    }
  ...
  async renameAgentRelease(agentId: string, release: any, body: any = {}, auth?: { organizationId?: string }) {
    const name = this.normalizeAgentName(agentId);
    const targetRelease = this.normalizeVersionValue(release);
    const nextName = String(body?.name || '').trim();
    if (!targetRelease) {
      throw new HttpException('Versao do agente invalida para renomear.', HttpStatus.BAD_REQUEST);
    }
    if (!nextName) {
      throw new HttpException('Informe um nome para a versao do agente.', HttpStatus.BAD_REQUEST);
    }

    const updated = await this.agentModel.findOneAndUpdate(
      { ...this.agentQuery(name, auth?.organizationId), 'releases.release': targetRelease },
      { $set: { 'releases.$.name': nextName } },
      { new: true },
    ).lean().exec();
    if (!updated) {
      throw new HttpException('Versao do agente nao encontrada.', HttpStatus.NOT_FOUND);
    }
  ...
  async overwriteAgentRelease(agentId: string, release: any, body: any = {}, auth?: CanvasFlowAuthContext) {
    const name = this.normalizeAgentName(agentId);
    const targetRelease = this.normalizeVersionValue(release);
    if (!targetRelease) {
      throw new HttpException('Versao do agente invalida para sobrescrita.', HttpStatus.BAD_REQUEST);
    }
    ...
    let flowNames: Record<string, string> = {};
    ...
    if (sourceRelease) {
      ...
      if (!source) {
        throw new HttpException(`Versao do agente r${sourceRelease} nao encontrada para origem.`, HttpStatus.NOT_FOUND);
      }
    ...
    if (body?.name !== undefined) set['releases.$.name'] = String(body.name || `Release ${targetRelease}`);
    if (body?.notes !== undefined) set['releases.$.notes'] = String(body.notes || '').trim();
  ...
  async deleteAgentRelease(agentId: string, release: any, auth?: { organizationId?: string }) {
    const name = this.normalizeAgentName(agentId);
    const targetRelease = this.normalizeVersionValue(release);
    if (!targetRelease) {
      throw new HttpException('Versao do agente invalida para exclusao.', HttpStatus.BAD_REQUEST);
    }
    ...
    if (this.normalizeVersionValue(agent.activeRelease) === targetRelease) {
      throw new HttpException('Nao e possivel excluir a versao ativa do agente. Ative outra versao antes.', HttpStatus.BAD_REQUEST);
    }
  ...
  async update(id: string, updateDto: UpdateCanvasFlowDto, auth?: { organizationId?: string }): Promise<any> {
    ...
    const { versions: _ignoredVersions, ...safeUpdateDto } = updateDto as any;
    const query = this.withOrganization({ _id: id }, auth?.organizationId);
    const updated = await this.model
