import { logEvent } from './observability/observability';
...
function parseBodyLimitBytes(value: unknown) {
...
export function getProductionSafetyFindings(env: NodeJS.ProcessEnv = process.env): ProductionSafetyFinding[] {
  if (!isProduction(env)) return [];

  const findings: ProductionSafetyFinding[] = [];
...
export function assertProductionSafety(env: NodeJS.ProcessEnv = process.env) {
  ...
  findings.forEach((finding) => {
    logEvent(finding.level === 'fail' ? 'error' : 'warn', `production_guard.${finding.code}`, {
